...
function buildButtons() {
  ...
  for (let i = 0; i < storedArray.length; i++) {
    ...
    let city = storedArray[i].cityName;
    coordObject = {
      lat: aLat,
      long: aLong,
      city: city,
    ...
    button.text(`${storedArray[i].cityName}`);
...
//     };
//     if (
//       storedArray[0] === undefined ||
//       cityInfo.cityLat !== storedArray[0].cityLat
//     ) {
...
function saveHistory(cityName, cityLat, cityLong) {
  const cityObj = { cityName, cityLat, cityLong };
  const currentHistory = getHistory();
  const index = currentHistory.findIndex(
    item => cityObj.cityName === item.cityName
  );
  currentHistory.unshift(cityObj);
  if (index < 0) {
    if (currentHistory.length > 6) {
      currentHistory.splice(-1, 1);
    }
  } else {
    currentHistory.splice(index + 1, 1);
  }
